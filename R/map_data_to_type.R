...
convert_tabname_col_prefix <- function(tab_names) {
  # gnarly rgx modified from BARD
  put_underscore_before <- str_extract(tab_names, "\\w{1}[a-z]")
  ...
  return(underscore_inserted)
}

fix_cols <- function(df, tab_names= c("CLTech","CL_Sub")){
  tibble(
...
# cluster_clean
# clusters_clean$CLTech
# convert_tabname_col_prefix(df_list=iccg_clean)
# convert_tabname_col_prefix(df_list=clusters_clean)
#
# tar_load(clusters_clean)
